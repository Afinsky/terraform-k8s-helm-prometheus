...
3. `eks-workloads` — a thin `terragrunt.hcl` wrapper around `modules/eks-workloads`, depending on `eks-cluster`'s outputs: IRSA-based controllers (aws-load-balancer-controller, external-dns, external-secrets), ingress-nginx, sample apps, and a `devops-admins` → `cluster-admin` `ClusterRoleBinding` (`rbac.tf`), native RBAC paired with the AWS access-policy admin association `eks-cluster` grants that same group. Also applies as `<account-alias>-devops-admin`.
