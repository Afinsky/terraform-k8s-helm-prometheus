...
2. `develop` — VPC, EKS cluster, IRSA-based controllers (aws-load-balancer-controller, external-dns, external-secrets), ECR, ACM, and the PLAT-101 EKS access lab (`eks-access-lab.tf`: namespaced RBAC via IAM Identity Center access entries for `payments-dev`/`payments-prod`/`search-dev`, consuming roles from `01-identity-center` by name pattern, not remote state or hardcoded ARNs). Applies as `lab-admin` — a consumer of the identity defined in layer 1, with no self-reference risk.
