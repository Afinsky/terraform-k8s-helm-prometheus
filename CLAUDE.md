...
A personal AWS/EKS homelab managed with Terraform via Terragrunt. One AWS account (`abotyan001`), one region (`us-east-1`), two layers. **Everything on the cluster is applied directly from this repo** — VPC/EKS, IRSA controllers (aws-load-balancer-controller, external-dns, external-secrets), ingress-nginx, and the sample apps (`app.tf`, `online-boutique.tf`) are all live `helm_release`/`kubernetes_manifest` resources here, not GitOps-reconciled.
...
- No `generate "provider"` block: `develop`'s kubernetes/helm providers are wired off live `module.eks` outputs, which Terragrunt can't template statically, so `provider.tf` is hand-written and committed as-is (in `modules/develop/`, see below — `01-identity-center` has no `terraform { source = ... }` and keeps its own `provider.tf` in the layer directory).

**`modules/`** (repo root): reusable Terraform modules, referenced from a layer's `terragrunt.hcl` via `terraform { source = "${get_repo_root()}/modules/<name>" }`. Currently just `modules/develop` — all of what used to live directly in `accounts/abotyan001/us-east-1/develop/` now lives here instead; that layer directory holds only `terragrunt.hcl` (the `source` + `inputs`), `state.hcl`, and the committed `.terraform.lock.hcl`. `01-identity-center` isn't modularized — its `.tf` files still live directly in the layer directory, since (unlike `develop`) nothing about it needs to be reused across accounts yet.
...
- `terraform-docs` regenerates `README.md` inputs/outputs tables (in `01-identity-center/` and in `modules/develop/`) between `BEGIN_TF_DOCS`/`END_TF_DOCS` markers on every commit (pre-commit hook + `.terraform-docs.yml`) — don't hand-edit those tables, edit the surrounding prose or the underlying `variables.tf`/`outputs.tf` instead. The `develop` layer directory's own `README.md` is hand-written prose (no `.tf` there to document) and isn't touched by this hook.
